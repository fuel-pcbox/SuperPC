...
locs decodemodrm(int seg, u8 modrm, bool word, bool segarg)
{
    ...
    switch(seg)
    {
        case SEG_CS: res.ea_seg = cs << 4; break;
        case SEG_DS: res.ea_seg = ds << 4; break;
        case SEG_ES: res.ea_seg = es << 4; break;
        case SEG_SS: res.ea_seg = ss << 4; break;
        case SEG_DEFAULT:
        {
            switch(modrm & 0xc7)
            {
                case 0x02: case 0x03: case 0x42: case 0x43: case 0x46: case 0x82: case 0x83: case 0x86: res.ea_seg = ss << 4; break;
                default: res.ea_seg = ds << 4; break;
            }
            break;
        }
    }
    ...
    case 0x40:
    {
        ...
        res.ea_offset = bx + si + tmp1;
        break;
    }
    case 0x41:
    {
        u8 tmp1 = RAM::rb(cs,ip+2);
        res.ea_offset = bx + di + tmp1;
        break;
    }
    case 0x42:
    {
        u8 tmp1 = RAM::rb(cs,ip+2);
        res.ea_offset = bp + si + tmp1;
        break;
    }
    case 0x43:
    {
        u8 tmp1 = RAM::rb(cs,ip+2);
        res.ea_offset = bp + di + tmp1;
        break;
    }
    case 0x44:
    {
        u8 tmp1 = RAM::rb(cs,ip+2);
        res.ea_offset = si + tmp1;
        break;
    }
    case 0x45:
    {
        u8 tmp1 = RAM::rb(cs,ip+2);
        res.ea_offset = di + tmp1;
        break;
    }
    case 0x46:
    {
        u8 tmp1 = RAM::rb(cs,ip+2);
        res.ea_offset = bp + tmp1;
        break;
    }
    case 0x47:
    {
        u8 tmp1 = RAM::rb(cs,ip+2);
        res.ea_offset = bx + tmp1;
        break;
    }
    case 0x80:
    {
    ...
    if(modrm < 0xC0)
    {
        ...
        res.srcptrvalid = false;
    }
    else res.srcptrvalid = true;
    return res;
}
...
void rtick()
{
    ...
    if(!i8080 && !halted)
    {
        ...
        case 0x0F:
        {
            if(type == intel8086 || type == intel8088)
            {
                printf("POP CS\n");
            ...
            /*else if(type>=intel286)
            {
                ...
                default:
                {
                    printf("Undefined 0F two-byte opcode %02x\n!",op2);
                    break;
                }
                }
            }*/
        ...
        case 0x88:
        {
            u8 modrm = RAM::rb(cs,ip+1);
            locs loc = decodemodrm(seg,modrm,false,false);
            if(loc.srcptrvalid) *loc.src8 = *loc.dst8;
            else
            {
                RAM::wb(loc.ea_seg + loc.ea_offset, *loc.dst8);
            }
            ip+=2;
        ...
        case 0x89:
        {
            u8 modrm = RAM::rb(cs,ip+1);
            locs loc = decodemodrm(seg,modrm,true,false);
            if(loc.srcptrvalid) *loc.src16 = *loc.dst16;
            else
            {
                RAM::wb(loc.ea_seg + loc.ea_offset, *loc.dst16 & 0xff);
                RAM::wb(loc.ea_seg + loc.ea_offset + 1, *loc.dst16 >> 8);
            }
            ip+=2;
        ...
        case 0x8A:
        {
            u8 modrm = RAM::rb(cs,ip+1);
            locs loc = decodemodrm(seg,modrm,false,false);
            if(loc.srcptrvalid) *loc.dst8 = *loc.src8;
            else *loc.dst8 = RAM::rb(loc.ea_seg + loc.ea_offset);
            ip+=2;
        ...
        case 0x8B:
        {
            u8 modrm = RAM::rb(cs,ip+1);
            locs loc = decodemodrm(seg,modrm,true,false);
            if(loc.srcptrvalid) *loc.dst16 = *loc.src16;
            else *loc.dst16 = RAM::rb(loc.ea_seg + loc.ea_offset) | (RAM::rb(loc.ea_seg + loc.ea_offset + 1) << 8);
            ip+=2;
        ...
        case 0xC0:
        {
            if(type <= necv20)
            {
                ...
                case 0x00:
                {
                    printf("ROL Eb,%02x modrm=%02x\n",op3,op2);
                    *loc.src8 = (*loc.src8 << op3) | (*loc.src8 >> (8 - op3));
                    break;
                }
                case 0x08:
                {
                    printf("ROR Eb,$02x modrm=%02x\n",op3,op2);
                    *loc.src8 = (*loc.src8 >> op3) | (*loc.src8 << (8 - op3));
                    break;
                }
                case 0x10:
                {
                    printf("RCL Eb,%02x modrm=%02x\n",op3,op2);
                    u8 tmp = *loc.src8;
                ...
                case 0x28:
                {
                    printf("SHR Eb,%02x modrm=%02x\n",op3,op2);
                    *loc.src8 >>= op3;
                    break;
                }
                case 0x30:
                {
                    printf("SAL Eb,%02x modrm=%02x\n",op3,op2);
                    *loc.src8 <<= op3;
                    break;
                }
                case 0x38:
                {
                    printf("SAR Eb,%02x modrm=%02x\n",op3,op2);
        ...
        case 0xC1:
        {
            if(type <= necv20)
            {
                ...
                case 0x00:
                {
                    printf("ROL Ev,%02x modrm=%02x\n",op3,op2);
                    *loc.src16 = (*loc.src16 << op3) | (*loc.src16 >> (16 - op3));
                    break;
                }
                case 0x08:
                {
                    printf("ROR Ev,$02x modrm=%02x\n",op3,op2);
                    *loc.src16 = (*loc.src16 >> op3) | (*loc.src16 << (16 - op3));
                    break;
                }
                case 0x10:
                {
                    printf("RCL Ev,%02x modrm=%02x\n",op3,op2);
                    u8 tmp = *loc.src16;
                    u8 tmp1 = flags & 0x0001;
                ...
                case 0x28:
                {
                    printf("SHR Ev,%02x modrm=%02x\n",op3,op2);
                    *loc.src16 >>= op3;
                    break;
                }
                case 0x30:
                {
                    printf("SAL Ev,%02x modrm=%02x\n",op3,op2);
                    *loc.src16 <<= op3;
                    break;
                }
                case 0x38:
                {
                    printf("SAR Ev,%02x modrm=%02x\n",op3,op2);
        ...
        case 0xC6:
        {
            ...
            locs loc = decodemodrm(seg,modrm,false,false);
            u8 tmp = RAM::rb(cs,ip+2);
            if(loc.srcptrvalid) *loc.src8 = tmp;
            else RAM::wb(loc.ea_seg + loc.ea_offset, tmp);
            ip+=3;
        ...
        case 0xC7:
        {
            ...
            locs loc = decodemodrm(seg,modrm,true,false);
            u16 tmp = RAM::rb(cs,ip+2)|(RAM::rb(cs,ip+3)<<8);
            if(loc.srcptrvalid) *loc.src16 = tmp;
            else
            {
                RAM::wb(loc.ea_seg + loc.ea_offset, tmp & 0xff);
                RAM::wb(loc.ea_seg + loc.ea_offset + 1, tmp >> 8);
            }
            ip+=4;
        ...
        case 0xCD:
        {
            ...
            if(tmp == 0x13) //TODO: HACK
            {
                ...
                case 0x00:
                {
                    ...
                    flags &= 0xFFFE;
                    ip+=2;
                    printf("Software Interrupt 0x13 function 0x00 triggered! HLEing!\n");
                    break;
                }
                case 0x02:
                {
                    ...
                    ip+=2;
                    printf("Software Interrupt 0x13 function 0x02 triggered! HLEing!\n");
                    break;
                }
                case 0x08:
                {
                    ...
                    dl = 1;
                    ip+=2;
                    printf("Software Interrupt 0x13 function 0x08 triggered! HLEing!\n");
                }
                }
            }
            else if(tmp == 0x16)
